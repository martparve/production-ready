...
Peter Steinberger compares multi-agent orchestration to "chess grandmasters playing multiple boards at once. They go to a board, they see what's there, make a decision. For some boards they stop for longer."[PE-24] The metaphor is apt but incomplete: chess grandmasters have decades of pattern recognition encoded in muscle memory. Multi-agent orchestration demands similar cognitive capabilities from engineers - the ability to hold multiple execution threads in mind, context-switch between agent outputs, and make judgment calls across parallel work streams. This is a genuinely new skill, and the burnout risk is real. An engineer managing six parallel agents is not doing one-sixth the work of writing code manually. They are doing a cognitively different kind of work that can be more exhausting than writing the code themselves.

Addy Osmani warns that parallel agents "compound all of these other problems."[PE-16] The math makes the mechanism concrete. If a single agent produces a defect on 15% of tasks, running ten agents in parallel does not give you a 15% defect rate across the batch - it gives you an 80% probability that at least one of the ten outputs contains a defect (1 - 0.85^10 = 0.80). At twenty parallel agents, the probability that the entire batch is clean drops to 4%. Scale the factory before fixing the production line and you do not get more output: you get near-certainty that every batch contains defects requiring human intervention. The right sequence is to improve the single-agent defect rate first, then scale horizontally.
