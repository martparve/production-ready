...
Steve Yegge describes an 8-level adoption spectrum running from Level 1 (no AI tools) through Level 6 (multiple agents running in parallel).[PE-29] Mapping your pilot team against this spectrum at the start and end of Phase 1 gives you a concrete before-and-after measurement that complements the go/no-go criteria above.
...
Shopify's rollout combined executive mandate with infrastructure investment. Farhan Thawar describes CEO Tobi Lutke's memo requiring "reflexive AI usage" across the organization[PE-07] - paired with the infrastructure to make it frictionless: two dozen MCP servers, a token usage leaderboard, and proxy-based usage tracking. The mandate alone would have generated resentment. The mandate plus infrastructure generated adoption. The lesson: top-down mandates work when they are backed by tooling that makes compliance easier than resistance.
