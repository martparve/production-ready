...
This push toward spec-first thinking reflects a broader trend. Addy Osmani at Google describes the same shift in his own practice: "I have more recently been focusing on the idea of spec-driven development, having a very clear plan of what it is that I want to build."[PE-16] Martin Fowler frames the historical context usefully: this looks like waterfall's return, but the economics are different.[PE-19] When iteration cost approaches zero, specifying upfront and regenerating cheaply when the spec changes becomes a rational strategy, not a throwback to heavyweight process. The distinction matters: this is not 1990s waterfall where a specification mistake took months to discover and a quarter of developer time to correct. It is specification paired with rapid feedback, where the cost of being wrong is a few minutes of agent time rather than a sprint of human time.
