...
The choice of retrieval mechanism matters as much as the choice of what to retrieve. Anthropic's internal team experimented with vector database RAG for codebase context and abandoned it.[PE-28] The problems were drift (embeddings go stale as code changes), staleness (newly added code does not appear in index until re-indexed), and poor embedding quality on code (vector similarity on source text does not reliably capture semantic relevance). Their replacement was agentic grep - the agent searches the codebase directly using structured search queries, getting fresh results with zero indexing lag. This is a strong practitioner signal: for code retrieval specifically, agentic search beats pre-indexed approaches in real production use, even when RAG looks more elegant on paper.
...
One architectural response to the attention problem in multi-agent systems is what Boris Cherny describes as "uncorrelated context windows."[PE-28] When two agents share the same context window, they share the same blind spots - they are likely to make the same mistakes on the same ambiguous instruction. Separate context windows mean independent failure modes: one agent might misinterpret a requirement while the other handles it correctly, and the divergence is detectable. Cherny's insight is that architectural independence is not just an efficiency pattern but a correctness pattern. Correlated failures are the hardest failures to catch because they produce confident, consistent wrong output rather than obvious error signals.
