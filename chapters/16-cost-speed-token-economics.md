...
Steve Yegge argues that token burn is the most important proxy metric for AI adoption: "The single most important proxy metric that you can have in a company today is token burn. Because what token burn says is your engineers are trying to do stuff."[PE-29] Shopify operationalized this: Farhan Thawar describes a "leaderboard where we actively celebrate the people who use the most tokens."[PE-07] This is the opposite of the cost-control instinct most finance teams have. Treating token spend as an investment rather than an expense changes organizational behavior: engineers experiment rather than self-censor. The caveats are real - token burn without quality metrics is vanity - but the directional bet is sound: an organization not burning tokens is not learning.
...
Kent Beck poses a useful thought experiment: "What would you do if cars were free? Things are going to be different."[PE-05] Applied to the factory: as code generation cost approaches zero, the economics of software invert. The expensive part of shipping software is no longer writing it - it is specifying what to write, validating that the output is correct, and maintaining trust in the production line. Organizations that budget primarily for implementation headcount are optimizing for the wrong cost. The factory's operating budget shifts from developer salaries to context engineering, validation infrastructure, and review capacity. This is not a prediction - it is the direction the cost curve is already moving.
