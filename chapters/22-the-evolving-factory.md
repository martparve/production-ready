...
When code generation cost approaches zero, the scarce resource shifts from technical execution to human judgment: the ability to decompose a problem, specify what to build, evaluate whether the output serves the user, and course-correct when it does not. Business acumen, domain intuition, interpersonal relationships - the skills required to figure out what humans actually want, including the things they cannot articulate - become the high-value capabilities. The factory serves human intent. Someone still has to define that intent clearly enough that the factory can act on it.

Recently, a CEO told me that product managers would soon be unnecessary at his company. He planned to build an "AI superbrain" - a knowledge base aggregating all communication, code, and internal data - that would make product decisions autonomously. My response: "I had to prompt you five times before you could articulate your thought clearly. How will your AI superbrain have high-quality inputs if there is no one to ask these questions?" The CEO looked annoyed, but the point stands. The bottleneck was never typing code. It was always knowing what to type. The factory makes the typing free. It does not make the knowing free.
