...
Kent Beck advocates an explicit annotation marking certain tests as inviolable: "I really want an immutable annotation that says, no, this is correct. And if you ever change this, you'll awaken in darkness forever."[PE-05] In practice, this means separating tests into two classes: agent-modifiable tests (the agent wrote them, the agent can rewrite them) and locked tests (human-authored contracts that define correct behavior and that no agent may alter). The locked tests are the validation layer's ground truth. If the agent's code fails a locked test, the code is wrong, period.
...
Addy Osmani describes a failure mode where progressive prompting causes context drift: "The two steps back pattern - you've used a couple of prompts to build up something. You give an LLM one more prompt and it happens to go in a completely different direction."[PE-16] For validation, this means the gate must verify not just that the latest change passes but that it has not silently broken what previous iterations built. Regression testing across the full agent session - not just the final diff - catches this pattern. Run the complete test suite after every agent iteration, not just at the end.
